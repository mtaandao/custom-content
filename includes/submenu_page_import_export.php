...
if ( !empty( $_POST ) && check_admin_referer( $this->plugin_slug . basename( __FILE__ ), $this->plugin_slug . '-nonce' ) ) {
	...
	if ( isset( $_POST['action-type'] ) ) {
		...
		if ( $_POST['action-type'] == 'export' ) {
			...
			if ( isset( $_POST['export'] ) ) {
				...
				/* Redirect */
				mn_redirect( admin_url( 'admin.php?page=' . $_GET['page'] . '&action=export' ) );
				exit;
			} else {
				
				/* Set the reponse message */
				$response['result'] = 'error';
				$response['message'][] = __( 'There is nothing to export!', 'my_portfolio_textdomain' );
		...
		/* Import action - validate & redirect */
			...
			if ( isset( $_POST['raw-import'] ) && $_POST['raw-import'] != '' ) {
				...
				if ( !is_array( $import_data ) ) {
					...
					/* Set the reponse message */
					$response['result'] = 'error';
					$response['message'][] = __( 'Invalid import data!', 'my_portfolio_textdomain' );
					set_transient( md5( $screen->id . '-response' ), $response, 30 );
					...
					/* Redirect */
					mn_redirect( admin_url( 'admin.php?page=' . $_GET['page'] . '&action=import' ) );
					exit;					
				}
				...
				/* Set the reponse message */
				$response['result'] = 'error';
				$response['message'][] = __( 'There is nothing to import!', 'my_portfolio_textdomain' );
				set_transient( md5( $screen->id . '-response' ), $response, 30 );
	...
	/* Import Page POST */
		...
		if ( !$temp_post_data ) { 
			mn_redirect( admin_url( 'admin.php?page=' . $_GET['page'] ) );
			exit;
		} else {
		...
		if ( isset( $_POST['import']['cpt'] ) && !empty( $_POST['import']['cpt'] ) ) {
			...
			$response['message'][] = sprintf( __( '%1$d custom post type item(s) has been imported.', 'my_portfolio_textdomain' ), $imported_cpt_cnt );
		}
		...
		if ( isset( $_POST['import']['portfolio'] ) && !empty( $_POST['import']['portfolio'] ) ) {
			...
			/* Set the reponse message */
			$response['result'] = 'success';
			$response['message'][] = sprintf( __( '%1$d portfolio item(s) has been imported.', 'my_portfolio_textdomain' ), $imported_pf_cnt );
		}
...
<div id="gwa-gopf-admin-wrap" class="wrap">
    ...
    <h2><?php _e( 'Import & Export', 'my_portfolio_textdomain' ); ?></h2>	
	<p></p>
	<?php
	...
	<form id="gwa-gopf-import-form" name="gwa-gopf-import-form" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>&noheader=true">
		...
		<!-- postbox -->
		<div class="postbox">
			<h3 class="hndle"><?php _e( 'Import & Export Data', 'my_portfolio_textdomain' ); ?><span class="gwmna-toggle"></span></h3>
			<div class="inside">
				<table class="form-table">
					<tr>
						<th class="gwa-gopf-w150"><div><?php _e( 'Select action', 'my_portfolio_textdomain' ); ?></div></th>
						<td class="gwa-gopf-w300">
							<select id="gwa-gopf-select" name="action-type" class="gwa-gopf-w250" data-parent="import-export">
								<option data-children="import" value="import"><?php _e( 'Import data', 'my_portfolio_textdomain' ); ?></option>
								<option data-children="export" value="export"><?php _e( 'Export data', 'my_portfolio_textdomain' ); ?></option>
							</select>
						</td>
						<td><p class="description"><?php _e( 'Import or export data.', 'my_portfolio_textdomain' ); ?></p></td>
					</tr>					

					<!-- import -->
					<tr class="gwa-gopf-group" data-parent="import-export" data-children="import">
						<th colspan="3"><?php _e( 'To import data open the file that contains demodata and copy its content to the textarea below and click to the "Save" button.', 'my_portfolio_textdomain' ); ?></th>
					</tr>
					<tr class="gwa-gopf-group" data-parent="import-export" data-children="import">
					...
					<tr class="gwa-gopf-group" data-parent="import-export" data-children="export">
						<th class="gwa-gopf-w150"><div><?php _e( 'Custom post type', 'my_portfolio_textdomain' ); ?></div></th>
						<td class="gwa-gopf-w300">
							<ul class="gwa-gopf-checkbox-list">
								<li><label><input type="checkbox" name="export[cpt][]" value="all" class="gwa-gopf-checkbox-parent"> <?php _e( 'All custom post types', 'my_portfolio_textdomain' ); ?> [&nbsp;.&nbsp;]<span></span></label>
									<ul class="gwa-gopf-checkbox-list">
						...
						<td><p class="description"><?php _e( 'Select the custom post types you would like to export.', 'my_portfolio_textdomain' ); ?></p></td>
					</tr>
					<?php endif; ?>
					<?php if ( isset( $portfolios ) && !empty( $portfolios ) ) : ?>
					<tr class="gwa-gopf-group" data-parent="import-export" data-children="export">
						<th class="gwa-gopf-w150"><div><?php _e( 'Portfolio', 'my_portfolio_textdomain' ); ?></div></th>
						<td class="gwa-gopf-w300">
							<ul class="gwa-gopf-checkbox-list">
								<li><label><input type="checkbox" name="export[portfolio][]" value="all" class="gwa-gopf-checkbox-parent"> <?php _e( 'All portfolios', 'my_portfolio_textdomain' ); ?> [&nbsp;.&nbsp;]<span></span></label>
									<ul class="gwa-gopf-checkbox-list">
						...
						<td><p class="description"><?php _e( 'Select the portfolios you would like to export.', 'my_portfolio_textdomain' ); ?></p></td>
					</tr>
					<?php endif; ?>
					<!-- /export -->
	...
	if ( !$temp_post_data ) {
		...
		<p><strong><?php _e( 'There is nothing to import!', 'my_portfolio_textdomain' ); ?> <a href="<?php echo esc_attr( admin_url( 'admin.php?page=' . $_GET['page'] ) ) ?>"><?php _e( 'Click here', 'my_portfolio_textdomain' ); ?></a> <?php _e( 'for Import & Export', 'my_portfolio_textdomain' ); ?></strong></p>
		</div>
		<?php
		exit;	
	}
	...
	<form id="gwa-gopf-import-form" name="gwa-gopf-import-form" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>&noheader=true">
		...
		<!-- postbox -->
		<div class="postbox">
			<h3 class="hndle"><?php _e( 'Import Data', 'my_portfolio_textdomain' ); ?><span class="gwmna-toggle"></span></h3>
			<div class="inside">
				<table class="form-table">
					<tr>
						<th colspan="3"><?php _e( 'Select the data to be imported and click to "Save" button.', 'my_portfolio_textdomain' ); ?></th>
					</tr>
					...
					<tr>
						...
						<td class="gwa-gopf-w300">
							<ul class="gwa-gopf-checkbox-list">
								<li><label><input type="checkbox" name="import[cpt][all]" value="<?php echo implode( ',', array_keys( $temp_post_data['cpt'] ) ); ?>" class="gwa-gopf-checkbox-parent"> <?php _e( 'All custom post types', 'my_portfolio_textdomain' ); ?> [&nbsp;.&nbsp;]<span></span></label>
									<ul class="gwa-gopf-checkbox-list">
					...
					<tr>
						...
						<td class="gwa-gopf-w300">
							<ul class="gwa-gopf-checkbox-list">
								<li><label><input type="checkbox" name="import[portfolio][all]" value="<?php echo implode( ',', array_keys( $temp_post_data['portfolio'] ) ); ?>" class="gwa-gopf-checkbox-parent"> <?php _e( 'All Portfolios', 'my_portfolio_textdomain' ); ?> [&nbsp;.&nbsp;]<span></span></label>
									<ul class="gwa-gopf-checkbox-list">
					...
					<tr>
						<th class="gwa-gopf-w150"><div><?php _e( 'Replace existing items?', 'my_portfolio_textdomain' ); ?></div></th>
						<th><label><input type="checkbox" name="replace" value="1" > <?php _e( 'Yes', 'my_portfolio_textdomain' ); ?></label></th>
				    	<td><p class="description"><?php _e( 'The existing items with same ids or slugs will be replaced with the imported ones if set, else a new copy will be created.', 'my_portfolio_textdomain' ); ?></p></td>
					</tr>					
					<?php endif; ?>
				</table>
			</div> 				
	...
	if ( $temp_post_data ) {
		...
		?>
		<div id="result" class="error">
		<p><strong><?php _e( 'There is nothing to export!', 'my_portfolio_textdomain' ); ?> <a href="<?php echo esc_attr( admin_url( 'admin.php?page=' . $_GET['page'] ) ) ?>"><?php _e( 'Click here', 'my_portfolio_textdomain' ); ?></a> <?php _e( 'for Import & Export', 'my_portfolio_textdomain' ); ?></strong></p>
		</div>
		<?php
		exit;
	}
	...
	<!-- postbox -->
	<div class="postbox">
		<h3 class="hndle"><?php _e( 'Export Data', 'my_portfolio_textdomain' ); ?><span class="gwmna-toggle"></span></h3>
		<div class="inside">
			<table class="form-table">
				<tr>
					<th><?php _e( 'Copy the content of the textarea below and save into file on your hard drive.', 'my_portfolio_textdomain' ); ?></th>
				</tr>
				<tr>
