	...
	if ( isset( $_POST['action-type'] ) ) {
		...
		if ( $_POST['action-type'] == 'import' ) {
			...
			if ( $new>0 ) {
				...
				$response['message'][] = __( 'New Templates & Styles has been successfully imported.', 'my_portfolio_textdomain' );
			} else {
				$response['result'] = 'error';
				$response['message'][] = __( 'No new templates or styles has been found.', 'my_portfolio_textdomain' );				
			}
		...
		/* Reset action */
			...
			/* Set the reponse message */
			$response['result'] = 'success';
			$response['message'][] = __( 'Templates & Styles has been successfully reseted.', 'my_portfolio_textdomain' );
			set_transient( md5( $screen->id . '-response' ), $response, 30 );
		...
		/* Edit action - redirect */
			...
			if ( isset( $_POST['item'] ) && !empty ( $_POST['item'] ) ) {
				...
				mn_redirect( admin_url( 'admin.php?page=' . $_GET['page'] . '&edit=' . $item_type . '&item=' . $item_id ) );
				exit;
			}			
		...
		/* Edit item action */
			...
			if ( isset( $_POST['item-type'] ) && ! empty( $_POST['item-type'] ) && isset( $_POST['item-id'] ) && ! empty( $_POST['item-id'] ) ) {
				if ( $_POST['item-type'] == 'template' ) {
					...
					/* Set the reponse message */
					$response['result'] = 'success';
					$response['message'][] = sprintf( __( '"%1$s" template has been successfully updated.', 'my_portfolio_textdomain' ), $templates[$_POST['item-id']]['name'] );
					...
					/* Set the reponse message */
					$response['result'] = 'success';
					$response['message'][] = sprintf( __( '"%1$s" style has been successfully updated.', 'my_portfolio_textdomain' ), $styles[$_POST['item-id']]['name'] );				
			...
			if ( isset( $_POST['item-type'] ) && ! empty( $_POST['item-type'] ) && isset( $_POST['item-id'] ) && ! empty( $_POST['item-id'] ) ) {
				if ( $_POST['item-type'] == 'template' ) {
					...
					if ( !isset( $new_templates[$_POST['item-id']]['data'] ) ) {
						$response['result'] = 'error';
						$response['message'][] = sprintf( __( '"%1$s" template file is missing.', 'my_portfolio_textdomain' ), $templates[$_POST['item-id']]['tpl_file'] );
					} else {
						$response['result'] = 'success';
						$response['message'][] = sprintf( __( '"%1$s" template has been successfully reseted.', 'my_portfolio_textdomain' ), $templates[$_POST['item-id']]['name'] );
					}
					...
					if ( !isset( $new_styles[$_POST['item-id']]['data'] ) ) {
						$response['result'] = 'error';
						$response['message'][] = sprintf( __( '"%1$s" stylesheet file is missing.', 'my_portfolio_textdomain' ), $styles[$_POST['item-id']]['css_file'] );
					} else {
						$response['result'] = 'success';
						$response['message'][] = sprintf( __( '"%1$s" style has been successfully reseted.', 'my_portfolio_textdomain' ), $styles[$_POST['item-id']]['name'] );							
					}
...
<div id="gwa-gopf-admin-wrap" class="wrap">
    ...
    <h2><?php _e( 'Template & Style Editor', 'my_portfolio_textdomain' ); ?></h2>	
	<p></p>
	<?php
	...
	<form id="gwa-gopf-editor-form" name="gwa-gopf-editor-form" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>&noheader=true">
		...
		<!-- postbox -->
		<div class="postbox">
			<h3 class="hndle"><?php _e( 'Template & Style Editor', 'my_portfolio_textdomain' ); ?><span class="gwmna-toggle"></span></h3>
			<div class="inside">
				<table class="form-table">
					<tr>
						<th class="gwa-gopf-w150"><div><?php _e( 'Select template or style', 'my_portfolio_textdomain' ); ?></div></th>
						<td class="gwa-gopf-w300">
							<select name="item" class="gwa-gopf-w250">
								<!-- Templates -->
								<?php if ( isset( $templates ) && !empty( $templates ) ) : ?>
								<optgroup label="<?php _e( 'Templates', 'my_portfolio_textdomain' ); ?>"></optgroup>
								<?php foreach( $templates as $tkey => $template ) : ?>
								<option value="template[<?php echo $tkey; ?>]"><?php echo $template['name']; echo !isset( $template['data'] ) ? ' ' . __( '(broken)', 'my_portfolio_textdomain' ) : ''; ?></option>
								<?php 
								endforeach;
								endif;
								?>
								...
								<optgroup label="<?php _e( 'Styles', 'my_portfolio_textdomain' ); ?>"></optgroup>
								<?php foreach( $styles as $skey => $style ) : ?>
								<option value="style[<?php echo $skey; ?>]"><?php echo $style['name']; echo !isset( $style['data'] ) ? ' ' . __( '(broken)', 'my_portfolio_textdomain' ) : ''; ?></option>
								<?php 
								endforeach;
								endif;
								?>								
						...
						<td><p class="description"><?php _e( 'Select template or style to edit.', 'my_portfolio_textdomain' ); ?></p></td>
					</tr>					
		...
		<p class="submit">
			<input type="submit" class="button-primary gwa-gopf-edit" value="<?php esc_attr_e( 'Edit', 'my_portfolio_textdomain' ); ?>" />
			<input type="button" class="button-secondary gwa-gopf-reset" value="<?php esc_attr_e( 'Reset All', 'my_portfolio_textdomain' ); ?>" />
			<input type="button" class="button-secondary gwa-gopf-import" value="<?php esc_attr_e( 'Import New', 'my_portfolio_textdomain' ); ?>" />
		</p>
	...
	if ( $_GET['edit'] == 'template' ) {
		if ( !isset( $templates[sanitize_key( $_GET['item'] )] ) ) {
			...
			<p><strong><?php _e( 'No template found!', 'my_portfolio_textdomain' ); ?> <a href="<?php echo esc_attr( admin_url( 'admin.php?page=' . $_GET['page'] ) ) ?>"><?php _e( 'Click here', 'my_portfolio_textdomain' ); ?></a> <?php _e( 'for Template & Style Editor', 'my_portfolio_textdomain' ); ?></strong></p>
			</div>
			<?php
			exit;		 
		} else {
	...
	if ( $_GET['edit'] == 'style' ) {
		if ( !isset( $styles[sanitize_key( $_GET['item'] )] ) ) {
			?>
			<div id="result" class="error">
			<p><strong><?php _e( 'No style found!', 'my_portfolio_textdomain' ); ?> <a href="<?php echo esc_attr( admin_url( 'admin.php?page=' . $_GET['page'] ) ) ?>"><?php _e( 'Click here', 'my_portfolio_textdomain' ); ?></a> <?php _e( 'for Template & Style Editor', 'my_portfolio_textdomain' ); ?></strong></p>
			</div>
			<?php
			exit;	 
		} else {
	...
	<form id="gwa-gopf-editor-form" name="gwa-gopf-editor-form" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>&noheader=true">
		...
		<!-- postbox -->
		<div class="postbox">
			<h3 class="hndle"><?php echo $item_type == 'template' ? sprintf( __( 'Edit Template: "%1$s"', 'my_portfolio_textdomain' ), $item_data['name'] ) :  sprintf( __( 'Edit style : "%1$s"', 'my_portfolio_textdomain' ), $item_data['name'] ) ; ?> <span class="gwmna-toggle"></span></h3>
			<div class="inside">
				<table class="form-table">
					<tr>
...
You can make further modications for each portfolio when creating a portfolio.', 'my_portfolio_textdomain' ); ?></th>
						<?php else : ?>
...
You can make further modications for each portfolio when creating a portfolio.', 'my_portfolio_textdomain' ); ?></th>						
					...
					<tr>
						...
						_e( '<p>This template or style seems to be broken.</p>', 'my_portfolio_textdomain' );
						printf( __('<p>The following file is missing: "<strong>%1$s</strong>".</p><p> Please check if the reqired files doest exist and click to "Reset".</p>', 'my_portfolio_textdomain' ), MY_PORTFOLIO_DIR . 'templates/templates/' .$req_file ); 
						endif;
						?>
						</th>
				    </tr>
		...
		<p class="submit">
			<?php if ( isset( $item_data['data'] ) ) : ?><input type="submit" class="button-primary gwa-gopf-edit-item" value="<?php esc_attr_e( 'Save', 'my_portfolio_textdomain' ); ?>" /><?php endif; ?>
			<input type="button" class="button-secondary gwa-gopf-reset-item" value="<?php esc_attr_e( 'Reset', 'my_portfolio_textdomain' ); ?>" />
		</p>
