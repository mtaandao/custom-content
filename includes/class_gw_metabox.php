	...
	public function print_custom_meta_box( $post, $metabox ) {
		...
		if ( isset( $metabox ) && !empty( $metabox ) ) {		
			...
			(function ($, undefined) {
				...
				$(function () {
					...
					$gopfMetabox.on('click', '.img-upload', function(e) {
						...
						/* New media uploader mn3.5+ */
						if ( typeof mn.media != 'undefined' ) {
						
							var file_frame = mn.media({
			...
			if ( $this->meta_box['id']==$metabox['args']['id'] ) {
				echo '<input type="hidden" name="' . $this->meta_box['id'] . '_nonce" value="' . mn_create_nonce( basename( __FILE__ ) ) . '" />',
						...
						case 'img-upload':
						 	if ( is_array( $meta ) ) {
								foreach ( $meta as $meta_item ) {
									echo '<input type="text" name="' . $field['id'] . '[]" id="' . $field['id']. '" value="' . ( $meta_item ? $meta_item : $field['default'] ) . '" class="' . $field['class'] . '" /><a href="#" class="img-upload button-secondary">' . __( 'Upload', 'my_portfolio_textdomain' ) . '</a>';
								}
							} else {
								echo '<input type="text" name="' . $field['id'] . '[]" id="' . $field['id']. '" value="' . ( $meta ? $meta : $field['default'] ) . '" class="' . $field['class'] . '" /><a href="#" class="img-upload button-secondary">' . __( 'Upload', 'my_portfolio_textdomain' ) . '</a>';
							}
							break;			
				...
				echo '<tr><th></th><td colspan="3" valign="top" align="left"><input name="save" type="submit" class="button-primary" value="' . esc_attr__( 'Update', 'my_portfolio_textdomain' ) . '"></td></tr></table>';
			}
	...
	public function save_custom_meta_box_data( $post_id ) {
		...
		if ( isset( $this->meta_box ) && !empty( $this->meta_box ) ) {
			if ( isset( $_POST[$this->meta_box['id'] . '_nonce'] ) ) {
				if ( !mn_verify_nonce( $_POST[$this->meta_box['id'] . '_nonce'], basename( __FILE__ ) ) ) { return $post_id; }
