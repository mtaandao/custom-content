...
(function ($, undefined) {
	...
	/ ---------------------------------------------------------------------- */			
			...
			if ( typeof mn.media != 'undefined' ) {
				...
				window.send_to_editor = function(html) {
					...
					var $html=$('<div />', { 'class':'media-html', 'html': html });
						...
						var imgID = imgClass.split('mn-image-')[1];
		...
		if ($colorPickerInput.length) {
			...
			if ($.fn.mnColorPicker) {
			  /* New colorpicker mn3.5+ */
			  $colorPickerInput.mnColorPicker();
			} else {
			   /* Old colorpicker */
	...
	/ ---------------------------------------------------------------------- */	
			...
			if ( typeof $this.data('ajax') != 'undefined' &&  $this.data('ajax')===true ) {
				if ($this.data('ajaxerrormsg')!=undefined) {
					$.ajax({  
						type: 'post', 
						url: ajaxurl,
						data: jQuery.param({ action: 'my_portfolio_plugin_menu_page', ajax: 'true' })+'&'+$this.serialize(),
						beforeSend: function () {
		...
		$pfForm.delegate('.gwa-gopf-reset-template, .gwa-gopf-reset-style', 'click', function() {
			...
			$.ajax({  
				type: 'get', 
				url: ajaxurl,
				data: jQuery.param({ action: 'my_portfolio_reset_template_style' })+'&'+item,
				beforeSend: function () {
